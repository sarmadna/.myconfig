...
cp $HOME/Downloads/myconfig/dotfiles/cavaconfig $HOME/.config/cava/config
...
wget https://github.com/romkatv/powerlevel10k-media/raw/master/MesloLGS%20NF%20Regular.ttf -P $HOME/Downloads
sudo mv $HOME/Downloads/*.ttf /usr/share/fonts
# SF fonts for display
git clone https://github.com/sahibjotsaggu/San-Francisco-Pro-Fonts.git $HOME/Downloads/SFPro
sudo cp -rv $HOME/Downloads/SFPro /usr/share/fonts
# Some other fonts
...
then
	...
	sh $HOME/Downloads/myconfig/fedora/gnome/gnome.sh
fi

if [ $(echo $XDG_CURRENT_DESKTOP) == "KDE" ]
then
	printf "${yellow}>>>${nc} ${cyan}Setting up KDE...${nc}\n"
	sh $HOME/Downloads/myconfig/fedora/kde/kde.sh
fi
